-- PostgreSQL schema for personal movie and series collection
-- Revised 2025-07-18 with:
--   • full support for series, seasons, and episodes
--   • normalized credits and metadata for series
--   • reverse-lookup indexes on all junction tables
--   • strict ISO code enforcement (char(2), no “Unknown” language)
--   • ENUMs for file_format, subtitle_format, aspect_ratio, audio_channels, series_status
--
...
CREATE TYPE public.series_status_enum    AS ENUM ('Returning Series','Ended','Canceled','In Production','Pilot');
...
CREATE TABLE public.series (
    ...
    first_air_date  date,
    last_air_date   date,
    status          public.series_status_enum,
    description     text,
    imdb_id         varchar(20),
    tmdb_id         integer,
    franchise_id    integer REFERENCES public.franchises(franchise_id) ON DELETE SET NULL,
...
CREATE TABLE public.seasons (
    ...
    series_id       integer NOT NULL REFERENCES public.series(series_id) ON DELETE CASCADE,
    ...
    created_at      timestamptz DEFAULT CURRENT_TIMESTAMP,
    updated_at      timestamptz DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT unique_series_season_number UNIQUE (series_id, season_number),
    CONSTRAINT seasons_tmdb_season_id_key UNIQUE (tmdb_season_id)
);
...
CREATE TABLE public.episodes (
    ...
    series_id       integer NOT NULL REFERENCES public.series(series_id) ON DELETE CASCADE, -- Denormalized for efficiency
    ...
    CONSTRAINT episodes_rating_check CHECK (rating IS NULL OR (rating >= 0 AND rating <= 10)),
    CONSTRAINT unique_season_episode_number UNIQUE (season_id, episode_number),
    CONSTRAINT episodes_imdb_id_key UNIQUE (imdb_id),
    CONSTRAINT episodes_tmdb_episode_id_key UNIQUE (tmdb_episode_id)
);
...
------------------------------------------------------------------
-- Media files (Episodes)
------------------------------------------------------------------
CREATE TABLE public.episode_files (
    file_id             integer GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    episode_id          integer NOT NULL REFERENCES public.episodes(episode_id) ON DELETE CASCADE,
    file_name           varchar(500) NOT NULL,
    file_path           text NOT NULL UNIQUE,
    added_to_library    timestamptz DEFAULT CURRENT_TIMESTAMP,
    file_format         public.file_format_enum,
    ...
    subtitle_embedded   boolean DEFAULT false,
    subtitle_external   boolean DEFAULT false,
    CONSTRAINT chk_ep_file_size_positive CHECK (file_size_mb IS NULL OR file_size_mb > 0)
);

CREATE TABLE public.episode_file_audio_tracks (
    track_id       integer GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    file_id        integer NOT NULL REFERENCES public.episode_files(file_id) ON DELETE CASCADE,
    audio_codec_id integer REFERENCES public.audio_codecs(codec_id) ON DELETE SET NULL,
    bitrate_kbps   integer,
    channels       public.audio_channels_enum,
    language_id    integer REFERENCES public.languages(language_id) ON DELETE SET NULL,
    track_order    integer NOT NULL,
    CONSTRAINT chk_ep_track_order_positive CHECK (track_order > 0)
);

CREATE TABLE public.episode_file_subtitles (
    subtitle_id     integer GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    file_id         integer NOT NULL REFERENCES public.episode_files(file_id) ON DELETE CASCADE,
    language_id     integer NOT NULL REFERENCES public.languages(language_id) ON DELETE RESTRICT,
    is_embedded     boolean DEFAULT false,
...
CREATE TABLE public.series_cast (
    ...
    billing_order  integer NOT NULL CHECK (billing_order > 0),
    role_type_id   integer REFERENCES public.role_types(role_type_id) ON DELETE SET NULL,
    CONSTRAINT unique_series_person_character UNIQUE (series_id, person_id, character_name)
);
...
CREATE TABLE public.episode_cast (
    ...
    billing_order  integer NOT NULL CHECK (billing_order > 0),
    role_type_id   integer REFERENCES public.role_types(role_type_id) ON DELETE SET NULL,
    CONSTRAINT unique_episode_person_character UNIQUE (episode_id, person_id, character_name)
);

CREATE TABLE public.episode_directors (
    episode_id integer NOT NULL REFERENCES public.episodes(episode_id) ON DELETE CASCADE,
    person_id  integer NOT NULL REFERENCES public.people(person_id)  ON DELETE CASCADE,
    PRIMARY KEY (episode_id, person_id)
);

CREATE TABLE public.episode_writers (
    episode_id     integer NOT NULL REFERENCES public.episodes(episode_id) ON DELETE CASCADE,
    person_id      integer NOT NULL REFERENCES public.people(person_id)  ON DELETE CASCADE,
    credit_type_id integer NOT NULL REFERENCES public.credit_types(credit_type_id) ON DELETE RESTRICT,
    PRIMARY KEY (episode_id, person_id, credit_type_id)
);

-- Metadata linkage tables
...
CREATE TABLE public.series_languages (
    series_id   integer NOT NULL REFERENCES public.series(series_id)   ON DELETE CASCADE,
    language_id integer NOT NULL REFERENCES public.languages(language_id) ON DELETE CASCADE,
...
CREATE TABLE public.series_countries (
    series_id  integer NOT NULL REFERENCES public.series(series_id)  ON DELETE CASCADE,
    country_id integer NOT NULL REFERENCES public.countries(country_id) ON DELETE CASCADE,
...
CREATE TABLE public.series_certifications (
    series_id        integer NOT NULL REFERENCES public.series(series_id) ON DELETE CASCADE,
    certification_id integer NOT NULL REFERENCES public.certifications(certification_id) ON DELETE CASCADE,
    PRIMARY KEY (series_id, certification_id)
);
...
CREATE INDEX idx_series_name_trgm   ON public.series USING gin (series_name public.gin_trgm_ops);
CREATE INDEX idx_episodes_name_trgm ON public.episodes USING gin (episode_name public.gin_trgm_ops);
...
CREATE INDEX idx_series_franchise_id ON public.series (franchise_id);
...
CREATE INDEX idx_episodes_series_id     ON public.episodes (series_id); -- For denormalized FK

-- Ordering
CREATE INDEX idx_movie_cast_billing     ON public.movie_cast (movie_id, billing_order);
CREATE INDEX idx_series_cast_billing    ON public.series_cast (series_id, billing_order);
CREATE INDEX idx_episode_cast_billing   ON public.episode_cast (episode_id, billing_order);
...
CREATE INDEX idx_series_creators_person     ON public.series_creators (person_id);
CREATE INDEX idx_series_cast_person         ON public.series_cast (person_id);
CREATE INDEX idx_episode_cast_person        ON public.episode_cast (person_id);
CREATE INDEX idx_episode_directors_person   ON public.episode_directors (person_id);
CREATE INDEX idx_episode_writers_person     ON public.episode_writers (person_id);
...
CREATE INDEX idx_episode_files_episode            ON public.episode_files (episode_id);
CREATE INDEX idx_episode_file_audio_tracks_file_id ON public.episode_file_audio_tracks (file_id);
CREATE INDEX idx_episode_file_subtitles_file_id    ON public.episode_file_subtitles (file_id);
...
CREATE INDEX idx_series_genres_genre_id        ON public.series_genres (genre_id);
CREATE INDEX idx_series_languages_language_id  ON public.series_languages (language_id);
CREATE INDEX idx_series_countries_country_id   ON public.series_countries (country_id);
CREATE INDEX idx_series_certifications_cert_id ON public.series_certifications (certification_id);
...
('US','United States'), ('GB','United Kingdom'), ('CA','Canada'), ('AU','Australia'), ('NZ','New Zealand'), ('IE','Ireland'), ('DE','Germany'), ('FR','France'), ('IT','Italy'), ('ES','Spain'), ('PT','Portugal'), ('BR','Brazil'), ('MX','Mexico'), ('AR','Argentina'), ('JP','Japan'), ('CN','China'), ('KR','South Korea'), ('IN','India'), ('RU','Russia'), ('NL','Netherlands'), ('BE','Belgium'), ('SE','Sweden'), ('NO','Norway'), ('DK','Denmark'), ('FI','Finland'), ('PL','Poland'), ('CZ','Czech Republic'), ('HU','Hungary'), ('RO','Romania'), ('BG','Bulgaria'), ('GR','Greece'), ('TR','Turkey'), ('IL','Israel'), ('TH','Thailand'), ('ID','Indonesia'), ('MY','Malaysia'), ('SG','Singapore'), ('PH','Philippines'), ('VN','Vietnam'), ('HK','Hong Kong'), ('TW','Taiwan'), ('CH','Switzerland'), ('AT','Austria'), ('ZA','South Africa'), ('EG','Egypt'), ('SA','Saudi Arabia'), ('AE','United Arab Emirates'), ('CL','Chile'), ('CO','Colombia'), ('PE','Peru'), ('VE','Venezuela'), ('UA','Ukraine'), ('HR','Croatia'), ('RS','Serbia'), ('SI','Slovenia'), ('SK','Slovakia'), ('LT','Lithuania'), ('LV','Latvia'), ('EE','Estonia'), ('IS','Iceland'), ('LU','Luxembourg'), ('MT','Malta'), ('CY','Cyprus')
ON CONFLICT (country_code) DO NOTHING;

-- Languages (ISO-639-1 + variants) – no "Unknown"
INSERT INTO public.languages (language_code, language_name) VALUES
('en','English'), ('en-US','English (US)'), ('en-GB','English (UK)'), ('fr','French'), ('fr-FR','French (FR)'), ('fr-CA','French (CA)'), ('de','German'), ('es','Spanish'), ('es-ES','Spanish (ES)'), ('es-MX','Spanish (MX)'), ('es-419','Spanish (Latin America)'), ('it','Italian'), ('pt','Portuguese'), ('pt-BR','Portuguese (BR)'), ('pt-PT','Portuguese (PT)'), ('ja','Japanese'), ('ko','Korean'), ('zh','Chinese'), ('zh-CN','Mandarin (Simplified)'), ('zh-TW','Mandarin (Traditional)'), ('zh-Hant','Mandarin (Hant)'), ('yue','Cantonese'), ('yue-Hant','Cantonese (Hant)'), ('ru','Russian'), ('ar','Arabic'), ('hi','Hindi'), ('nl','Dutch'), ('sv','Swedish'), ('no','Norwegian'), ('nb','Norwegian Bokmål'), ('nn','Norwegian Nynorsk'), ('da','Danish'), ('fi','Finnish'), ('pl','Polish'), ('cs','Czech'), ('hu','Hungarian'), ('ro','Romanian'), ('bg','Bulgarian'), ('el','Greek'), ('tr','Turkish'), ('he','Hebrew'), ('th','Thai'), ('vi','Vietnamese'), ('id','Indonesian'), ('ms','Malay'), ('tl','Tagalog'), ('uk','Ukrainian'), ('hr','Croatian'), ('sr','Serbian'), ('sl','Slovenian'), ('sk','Slovak'), ('lt','Lithuanian'), ('lv','Latvian'), ('et','Estonian'), ('is','Icelandic'), ('ga','Irish'), ('cy','Welsh'), ('gd','Scottish Gaelic'), ('eu','Basque'), ('ca','Catalan'), ('gl','Galician'), ('sq','Albanian'), ('mk','Macedonian'), ('bs','Bosnian'), ('mt','Maltese'), ('af','Afrikaans'), ('sw','Swahili'), ('am','Amharic'), ('bn','Bengali'), ('gu','Gujarati'), ('kn','Kannada'), ('ml','Malayalam'), ('mr','Marathi'), ('ne','Nepali'), ('pa','Punjabi'), ('si','Sinhala'), ('ta','Tamil'), ('te','Telugu'), ('ur','Urdu'), ('fa','Persian'), ('kk','Kazakh'), ('ky','Kyrgyz'), ('uz','Uzbek'), ('az','Azerbaijani'), ('ka','Georgian'), ('hy','Armenian'), ('mn','Mongolian'), ('bo','Tibetan'), ('km','Khmer'), ('lo','Lao'), ('my','Burmese')
ON CONFLICT (language_code) DO NOTHING;

-- Video codecs
INSERT INTO public.video_codecs (codec_name) VALUES
('H.264'), ('H.265'), ('HEVC'), ('AV1'), ('VP9'), ('VP8'), ('MPEG-2'), ('MPEG-4'), ('DivX'), ('XviD'), ('WMV'), ('ProRes'), ('DNxHD'), ('DNxHR'), ('Theora'), ('VC-1')
ON CONFLICT (codec_name) DO NOTHING;

-- Audio codecs
INSERT INTO public.audio_codecs (codec_name) VALUES
('AAC'), ('MP3'), ('DTS'), ('DTS-HD'), ('DTS-HD MA'), ('DTS:X'), ('Dolby Digital'), ('Dolby Digital Plus'), ('Dolby TrueHD'), ('Dolby Atmos'), ('FLAC'), ('PCM'), ('LPCM'), ('Opus'), ('Vorbis'), ('AC3'), ('E-AC3'), ('WMA'), ('ALAC'), ('APE'), ('WAV')
ON CONFLICT (codec_name) DO NOTHING;

-- Video resolutions
INSERT INTO public.video_resolutions (resolution_name, width_pixels, height_pixels) VALUES
('360p', 640, 360), ('480p', 854, 480), ('576p', 1024, 576), ('720p', 1280, 720), ('900p', 1600, 900), ('1080p', 1920, 1080), ('1440p', 2560, 1440), ('4K', 3840, 2160), ('5K', 5120, 2880), ('6K', 6144, 3456), ('8K', 7680, 4320), ('IMAX', 4096, 3072)
ON CONFLICT (resolution_name) DO NOTHING;

-- Source media types
INSERT INTO public.source_media_types (source_type_name) VALUES
('Blu-ray'), ('4K Blu-ray'), ('DVD'), ('Digital'), ('Web-DL'), ('WEB-Rip'), ('TV-Rip'), ('HDTV'), ('Cam'), ('Telesync'), ('Telecine'), ('Screener'), ('VOD'), ('iTunes'), ('Amazon'), ('Netflix'), ('Disney+'), ('HBO Max'), ('Theatrical')
ON CONFLICT (source_type_name) DO NOTHING;

-- Credit types
INSERT INTO public.credit_types (credit_type_name) VALUES
('Screenplay'), ('Story'), ('Adaptation'), ('Original Story'), ('Novel'), ('Characters'), ('Treatment'), ('Book'), ('Play'), ('Article'), ('Comic Book'), ('Short Story'), ('Based on'), ('Concept'), ('Idea'), ('Teleplay')
ON CONFLICT (credit_type_name) DO NOTHING;

-- Role types
INSERT INTO public.role_types (role_name) VALUES
('Lead'), ('Supporting'), ('Cameo'), ('Voice'), ('Narrator'), ('Uncredited'), ('Special Appearance'), ('Guest Star'), ('Archive Footage'), ('Motion Capture'), ('Stunt Double'), ('Body Double')
ON CONFLICT (role_name) DO NOTHING;

-- Genres
INSERT INTO public.genres (genre_name) VALUES
('Action'), ('Adventure'), ('Animation'), ('Comedy'), ('Crime'), ('Documentary'), ('Drama'), ('Family'), ('Fantasy'), ('History'), ('Horror'), ('Music'), ('Mystery'), ('Romance'), ('Science Fiction'), ('TV Movie'), ('Thriller'), ('War'), ('Western'), ('Biography'), ('Film-Noir'), ('Game-Show'), ('Musical'), ('News'), ('Reality-TV'), ('Sport'), ('Talk-Show'), ('Adult')
ON CONFLICT (genre_name) DO NOTHING;

------------------------------------------------------------------
-- Views for consolidation
------------------------------------------------------------------
...
SELECT
    ...
    COALESCE(
        ARRAY_AGG(DISTINCT cert.certification_code || ' (' || ctry.country_code || ')') FILTER (WHERE cert.certification_code IS NOT NULL),
        ARRAY[]::varchar[]
    ) AS certifications,
    COALESCE(
...
CREATE OR REPLACE VIEW public.series_details AS
SELECT
    s.series_id,
    s.series_name,
    s.original_name,
    ...
    COALESCE(
        (SELECT ARRAY_AGG(p.full_name)
         FROM public.series_creators sc
         JOIN public.people p USING (person_id)
         WHERE sc.series_id = s.series_id),
        ARRAY[]::varchar[]
    ) AS creators,
    (SELECT COUNT(*) FROM public.seasons se WHERE se.series_id = s.series_id) AS season_count,
    (SELECT COUNT(*) FROM public.episodes e WHERE e.series_id = s.series_id) AS episode_count
FROM public.series s
LEFT JOIN public.franchises f             ON f.franchise_id   = s.franchise_id
LEFT JOIN public.series_genres sg         ON sg.series_id     = s.series_id
LEFT JOIN public.genres g                 ON g.genre_id       = sg.genre_id
LEFT JOIN public.series_languages sl      ON sl.series_id     = s.series_id
LEFT JOIN public.languages lang           ON lang.language_id = sl.language_id
LEFT JOIN public.series_countries sc      ON sc.series_id     = s.series_id
LEFT JOIN public.countries ctry           ON ctry.country_id  = sc.country_id
LEFT JOIN public.series_certifications sct ON sct.series_id    = s.series_id
LEFT JOIN public.certifications cert      ON cert.certification_id = sct.certification_id
GROUP BY s.series_id, f.franchise_name;


CREATE OR REPLACE VIEW public.episode_details AS
SELECT
    e.episode_id,
    e.episode_name,
    e.episode_number,
    ...
    COALESCE(
        (SELECT ARRAY_AGG(p.full_name)
         FROM public.episode_directors ed
         JOIN public.people p USING (person_id)
         WHERE ed.episode_id = e.episode_id),
        ARRAY[]::varchar[]
    ) AS directors,
    COALESCE(
        (SELECT ARRAY_AGG(p.full_name || ' (' || ct.credit_type_name || ')')
         FROM public.episode_writers ew
         JOIN public.people p USING (person_id)
         JOIN public.credit_types ct USING (credit_type_id)
         WHERE ew.episode_id = e.episode_id),
        ARRAY[]::varchar[]
    ) AS writers,
    (SELECT COUNT(*) FROM public.episode_files ef WHERE ef.episode_id = e.episode_id) AS file_count
FROM public.episodes e
JOIN public.seasons se ON e.season_id = se.season_id
JOIN public.series s   ON e.series_id = s.series_id;